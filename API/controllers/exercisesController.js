...
exercisesController.readById = async (req, res) => {
  const userId = req.user;
  const id =
    typeof parseInt(req.params.id) === "number"
...
exercisesController.create = async (req, res) => {
  ...
  const name =
    typeof req.body.name === "string" && req.body.name.trim().length > 0
      ...
      : false;
  const desc =
    typeof req.body.desc === "string" && req.body.desc.trim().length > 0
      ? req.body.desc
      : false;
  const defaultSeries =
    typeof req.body.defaultSeries === "number" ? req.body.defaultSeries : false;
  const defaultReps =
    typeof req.body.defaultReps === "number" ? req.body.defaultReps : false;
  const defaultRepsType =
    typeof req.body.defaultRepsType === "string" &&
    req.body.defaultRepsType.trim().length > 0
      ? req.body.defaultRepsType
      : false;
  const defaultEquip =
    typeof req.body.defaultEquip === "string" &&
    req.body.defaultEquip.trim().length > 0
      ? req.body.defaultEquip
      : false;
  const defaultWeight =
    typeof req.body.defaultWeight === "string" && req.body.defaultWeight.trim().length > 0
    ? req.body.defaultWeight
    : false;
  const video1 =
    typeof req.body.video1 === "string" && req.body.video1.trim().length > 0
      ? req.body.video1
      : false;
  const video2 =
    typeof req.body.video2 === "string" && req.body.video2.trim().length > 0
      ? req.body.video2
      : false;
  const comment =
  ...
  if (name && userId) {
    ...
    const newExercise = await exercisesService.create(exercise);
    // Return data
    res.status(200).json({
      success: true,
...
exercisesController.update = async (req, res) => {
  ...
  const defaultEquip =
    ...
    req.body.defaultEquip.trim().length > 0
      ...
      : false;
  const defaultWeight =
    typeof req.body.defaultWeight === "number" ? req.body.defaultWeight : false;
  const video1 =
    typeof req.body.video1 === "string" && req.body.video1.trim().length > 0
      ? req.body.video1
      : false;
...
exercisesController.delete = async (req, res) => {
  ...
  // Check if required data exists
  const id =
    typeof parseInt(req.body.id) === "number" ? parseInt(req.body.id) : false;
  if (id && userId) {
